...
st.markdown("<p className='text-slate-300'>Let's fix your prediction issue step by step</p>", unsafe_allow_html=True)
...
""",
    ...
    # Feature mismatch warning with helpful message
    ...
    with leftpane:
        ...
        if matches_df is not None:
            ...
            st.caption("Load all_matches.csv to see head-to-head data.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with rightpane:
        ...
        if matches_df is not None:
            ...
            st.caption("Load all_matches.csv to see recent matches.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    try:
        if up_npy is not None:
            ...
            if arr.shape != (seq_len, n_feat):
                ...
                X_seq = arr
                st.success(f"✅ NPY loaded: shape {arr.shape}")
            ...
            if arr.shape != (seq_len, n_feat):
                ...
                X_seq = arr
                st.success(f"✅ CSV loaded: shape {arr.shape}")
    except Exception as e:
    ...
    if X_seq is not None:
        ...
        if st.button("🎯 Predict from custom sequence", type="primary", use_container_width=True):
            try:
                ...
                st.markdown(
                    "<div class='scoreboard'><h1>🎯 Prediction Result</h1></div>",
                    unsafe_allow_html=True,
                )
                render_prob_bars(probs, colors)
                ...
                # Betting recommendation
                if conf > 70:
                    st.info("💰 **STRONG BET** - High confidence prediction")
                elif conf > 60:
                    st.info("⚖️ **MODERATE BET** - Reasonable confidence")
                elif conf > 50:
                    st.warning("⚠️ **WEAK BET** - Low confidence")
                else:
                    st.error("❌ **AVOID** - Very uncertain prediction")
...
with tabs[2]:
    ...
    for key, value in summary_data.items():
        st.text(f"{key}: {value if value else 'None'}")
    ...
    st.text(f"Features: {n_feat}")
    
    if label_encoder and hasattr(label_encoder, "classes_"):
        st.text(f"Classes: {list(label_encoder.classes_)}")
    
    if team_encoder and hasattr(team_encoder, "classes_"):
        st.text(f"Teams encoded: {len(team_encoder.classes_)}")
        with st.expander("📋 View all teams"):
            teams_list = list(team_encoder.classes_)
            cols = st.columns(3)
            for i, team in enumerate(sorted(teams_list)):
                cols[i % 3].text(team)
